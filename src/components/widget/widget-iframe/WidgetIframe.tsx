...
export const WidgetIframe = ({id, data, mode}: GenericWidgetProps) => {
    ...
    if (isBeingEdited) {
        ...
        return <WidgetEditer key={id} id={id} data={data} onSaveWidget={onSaveWidget}/>
    }
    
    return <div className="widget widget-iframe-container" key={id}>
            {mode==='EDITING' && <NavLink icon="fa-pen" style={{position: 'absolute', right: '10px'}} onClick={() => {setIsBeingEdited(true)}} /> }
            <iframe title={data?.url} className="widget-iframe" scrolling="no"  style={{overflow:"hidden", width:"100%",border:0,margin:0,padding:0}} 
                src={data?.url}
